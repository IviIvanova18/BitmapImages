...
List_Multiple_Contours create_List_Multi_empty(){
	List_Multiple_Contours L = {0, NULL};
	return L;
}
...
List_Multiple_Contours add_next_Contour(List_Multiple_Contours *List, Contour c){
	...
	if (List->count == 0)
	{
		...
		node->next_contour =NULL;
	}
	else
	{

		List->tail->next_contour = node;
		List->tail = node;
		node->next_contour = NULL;
	}
	List->count++;
...
{
	Node_Segment *el;
	el = (Node_Segment *)malloc(sizeof(Node_Segment));
	if (el==NULL)
	{
		fprintf(stderr, "create_element_List_Segemnt : allocation impossible\n");
		exit(-1);
	}
	el->data.P1 = v.P1;
	el->data.P2 = v.P2;

	el->next_segment = NULL;
	return el;
}

List_Segments create_List_Segment_vide()
{
	List_Segments L = {0, NULL, NULL};
	return L;
}
...
List_Segments add_element_List_Segments(List_Segments* L, Segment e)
{
	...
	el = create_element_List_Segment(e);
